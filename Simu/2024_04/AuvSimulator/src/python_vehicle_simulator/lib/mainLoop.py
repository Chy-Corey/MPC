...
def printSimInfo():
    ...
    print('---------------------------------------------------------------------------------------')
    print('1 - Remus 100: PID depthHeadingAutopilot')
    print('---------------------------------------------------------------------------------------')
    print('2 - Remus 100: MPC depthControl with modelSim')
    print('---------------------------------------------------------------------------------------')
    print('3 - Remus 100: MPC depthControl without modelSim')
    print('---------------------------------------------------------------------------------------')
...
def simulate(n, sample_time, vehicle):
    ...
    eta = np.array([0, 0, 30, 0, 0, 0], float)  # position/attitude, user editable；x，y，z，phi，theta，psi
    ...
    for i in range(0, n + 1):
        ...
        if vehicle.controlMode == 'mpcDepthControl':
            u_control = vehicle.MPC_6DOF(eta, nu, sample_time, u_actual, pre_step, i)
...
# def simulate_modelsim(n, sample_time, vehicle):
#     DOF = 6  # degrees of freedom 六个自由度：x，y，z，phi(p, roll)，theta(q, pitch)，psi(r, yaw)
#     t = 0  # initial simulation time
#     pre_step = 5
#
...
#         elif vehicle.controlMode == 'stepInput':
#             u_control = vehicle.stepInput(t)
#         elif vehicle.controlMode == 'mpcDepthControl':
#             u_control = vehicle.MPC_6DOF(eta, nu, sample_time, u_actual, pre_step)
#
#             # Store simulation data in simData
#         signals = np.append(np.append(np.append(eta, nu), u_control), u_actual)
#         simData = np.vstack([simData, signals])
