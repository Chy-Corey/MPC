...
<<<<<<< HEAD
通过状态观测器可观测升降舵的状态，并且可在 t 时刻预测 t+1 时刻的状态，就可以用预测的状态计算得到 t+1 时刻的输出（预测输出）。如果预测的输出和实际测量的输出不相等，说明存在扰动，可能有故障。（通过这种形式可以观测到 mpc 得到的控制输入的实际作用效果）
=======
...
>>>>>>> 214aa3d8bfcb16c67c47c4b54ae87bc4b035353e

为什么不直接用 mpc 的控制输入减去 $y_{el}$ ？可能因为本文考虑了执行器的作用效率，给定的控制输入可能需要一段时间，执行器才能完成任务。
...
使用积分形式建模，增广于 mpc 状态方程中，可以使用 kalman 滤波对 mpc 的状态方程进行估计，从而减小扰动的影响。也可以
