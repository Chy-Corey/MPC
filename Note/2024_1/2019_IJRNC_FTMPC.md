...
首先，本文的 MPC 控制器不在预测器（优化问题）中模拟传感器和滤波器的动态，以减少决策变量的数量（从而减少计算时间）。因此，观测器监测 $e_{nz}=n_z^m-n_z^p$ ，即测量值与预测值之间的不匹配。
...
诊断重构的目的是帮助 FD 模块了解干扰故障的根本原因。MPC 控制器会在 $t_{fi}$ 时刻检查 e𝛿ei 的符号，以决定是修改 𝛿ei 还是修改 𝛿ei。
的符号，以决定是修改 𝛿ei 还是 𝛿ei，即修改第 i 个电梯的上限值还是下限值。
或 𝛿ei，即修改第 i 个电梯的上限值还是下限值。需要注意的是，MPC 问题公式中的这一修改只影响 g𝜃 的定义（即用于生成人工参考信号的参数𝜃 的可行区域）。我们的想法是将受干扰的电梯边界暂时设置为一个收紧值 𝛿f
ei±𝛾，其中 𝛿f ei 是电梯在 tfi 时刻的测量值，𝛾 是一个正常数，应调整到合适的水平。
是一个正常数，应调整得足够小，以保持控制器的性能，但同时又要足够大，以允许残余信号的大小超过预定的阈值 Jth
i 的阈值。注意
根据图 4 的描述，正负（±）符号取决于 MPC 修改的界限。MPC 会在𝜏 样本中保持这个新的𝛾 加密边界。一方面，𝜏 必须选得足够大，以确保控制指令 u 有时间适应更新后的（就可行区域而言）参数 𝜃；另一方面，𝜏 必须选得足够小，以保持性能（尤其是在误报或故障卡住的情况下）。将 𝜏 设置为与预测范围 N 成比例是合理的。
