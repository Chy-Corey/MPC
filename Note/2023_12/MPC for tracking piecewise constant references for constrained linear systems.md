...
当 $x=x_s^*$ 时，系统已经达到稳态（初始状态就是稳态），那么优化问题可以表示为：
$$
\begin{align}
P_N(x_s^*,\hat{x}_S) &=V_N^*(x_s^*,\hat{x}_s)\\
&=||x_s^*-\hat{x}_s||_T^2
\end{align}
$$


根据引理 1 ：$x=x_s^*\in O_{\infty}(\bar{x}_s)$ ，由于在不变集中，所以 $u=K(x-\bar{x}_s)+u_s$ 是可行的，不会违反约束，那么根据引理 2，可以得出：
$$
V_N(x_s^*,\hat{x}_s,\bar{u},\bar{\theta})\leq ||x_s^*-\bar{x}_s||^2_P+||\bar{x}_s-\hat{x}_s||^2_T
$$
根据定理 1 的第二点，可以从上式得到：
$$
V_N(x_s^*,\hat{x}_s,\bar{u},\bar{\theta}) < || x_s^*-\hat{x}_s ||^2_T=V_N^*(x_s^*,\hat{x}_s)
$$
然而 $V_N^*(x_s^*,\hat{x}_s)$ 是最优的，不可能有比它还小的优化方案，假设不成立，所以 $x=x_s^*=\hat{x}_s$ 。



**定理证明：**

变量表示：$(u^*(k),\theta^*(k))$ 表示 k 时刻的最优解。 $x_s^*(k)$ 和 $u_s^*(k)$ 表示最优稳态和控制输入，由 $\theta^*(k)$ 得到。

$x^*(i;k)$ 表示开环预测的最优值：$x^*(i;k)=Ax^*(i-1;k)+Bu^*(i-1;k)$ ，$x^*(0;k)=x_k$

**可解性(feasibility)：**

假设在时间 K 时 $x_k\in X_N$ ，$V_N^*(x_k,\hat{x}_s)$ 的解为 $(u^*(k),\theta^*(k))$ ，k+1 时刻的状态为 $x_{k+1}$ ，考虑开环情况 $\theta(k+1)=\theta^*(k)$ ，控制序列：
$$
u(k+1)=\{u^*(1;k),\dots, u^*(N-1;k),K(x^*(N;k-x^*_s(k))+u_s^*(k)) \}
$$
这也是可行的，因为仍然在不变集中，满足系统约束。
...
$$
\begin{align}
V_N^*(x_{k+1},\hat{x}_s) &\leq V_N(x_{k+1},\hat{x}_s,u(k+1),\theta(k+1))\\
	&\leq V_N^*(x_{k},\hat{x}_s)-||x_k-x_s^*(k)||^2_Q
\end{align}
$$
所以成本函数会逐渐减小，直到 $||x_k-x_s^*(k)||^2_Q=0$ ，当等于 0 成立时，根据引理 3，$x=\hat{x}_S=x_s^*$ 。
